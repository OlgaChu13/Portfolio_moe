...
export function initStrands(container, options = {}) {
  ...
  const program = new Program(gl, {
    ...
    uniforms: {
      ...
      uColors: { value: buildPalette(state.colors) },
      uColorCount: { value: Math.min(state.colors.length, MAX_COLORS) },
      uStrandCount: { value: Math.min(state.count, MAX_STRANDS) },
  ...
  const update = (t) => {
    ...
    program.uniforms.uColors.value = buildPalette(state.colors);
    program.uniforms.uColorCount.value = Math.min(state.colors.length, MAX_COLORS);
    program.uniforms.uStrandCount.value = Math.min(Math.max(Math.round(state.count), 1), MAX_STRANDS);
    program.uniforms.uSpeed.value = state.speed;
    program.uniforms.uAmplitude.value = state.amplitude;
    program.uniforms.uWaviness.value = state.waviness;
    program.uniforms.uThickness.value = state.thickness;
    program.uniforms.uGlow.value = state.glow;
    program.uniforms.uTaper.value = state.taper;
    program.uniforms.uSpread.value = state.spread;
    program.uniforms.uHueShift.value = state.hueShift;
    ...
    program.uniforms.uSaturation.value = state.saturation;

    if (state.glass) {
      renderer.render({ scene: mesh, target: renderTarget });
      glassProgram.uniforms.uScene.value = renderTarget.texture;
      glassProgram.uniforms.uRefraction.value = state.refraction;
      glassProgram.uniforms.uDispersion.value = state.dispersion;
      glassProgram.uniforms.uRadius.value = 0.46 * state.glassSize;
